...
## 🐚🔴 IN zsh `path` E' LA VARIABILE SPECIALE LEGATA A `PATH`: UN `read -r path` SVUOTA IL PATH DELLA SHELL, E I COMANDI CHE MUOIONO DOPO STAMPANO **VUOTO**, NON UN ERRORE (w1, 2026-09-26, correggendo una MIA diagnosi)
Stamattina avevo messo a verbale come causa di un loop rotto *«`git` non si risolveva nel subshell, rc=127, pur essendo in PATH»*. **Era il SINTOMO.** La causa, isolata da w1 nei due versi: in zsh **`path` e' l'array speciale legato a `PATH`**, quindi `while IFS= read -r path; do …` **azzera `PATH` a ogni riga**. Misurato: col nome `path` ⇒ `PATH` **vuoto**; col nome `p` ⇒ **1381 caratteri intatti**, stesso loop.
🔴 **E il secondo giro ha morso molto piu' largo del primo:** dopo il loop sono morti **`awk`, `sort`, `comm`, `sed`, `wc`, `tr`, `cat`**, e lo step successivo ha stampato **conteggi VUOTI** — che **si leggono come uno spazio, non come un errore**. Nessun rc≠0 in faccia, nessun messaggio: solo numeri che mancano.
🥇 **L'unica cosa che l'ha smascherato e' la regola gia' scritta qui: un risultato UNIFORME su tutto il campione (0/N o N/N) accusa lo STRUMENTO prima del dato.** Senza quella, il referto sarebbe uscito con delle colonne vuote e qualcuno le avrebbe lette come zeri.
🥇🥇 **E la lezione piu' grossa e' sulla CURA, non sulla causa: il mio "fix" di stamattina — invocare `git` col path assoluto — MASCHERAVA la causa invece di toglierla.** Funzionava per `git` e lasciava armata la trappola per **ogni altro binario** del resto dello script, che infatti e' saltata al giro dopo, piu' in grande. ⇒ **una cura che sopprime il sintomo dove l'hai visto, senza nominare il meccanismo, RIARMA la trappola altrove** — ed e' peggio di nessuna cura, perche' chiude il caso.
🔑 **Regola operativa: in uno script che girera' sotto zsh non si usano come variabili i nomi speciali** — `path`, `PATH`, `cdpath`, `fpath`, `manpath`, `status`, `argv`, `prompt`, `options`, `signals`, `histchars`. Il piu' insidioso e' `path` **perche' e' esattamente il nome che viene naturale in un loop sui file.** Usa `p`, `f`, `fp`.
🪞 *Terza trappola zsh a verbale in questo file, dopo `$VAR:refs/…` letto come modificatore `:r` e `for f in $files` che non splitta. Le tre hanno la stessa forma: **zsh non sbaglia, fa una cosa DIVERSA da bash e non lo dice**, e il risultato e' sempre un output che sembra un dato.*

## 🧭 UN SOGGETTO DI CONTROLLO PUO' **PERDERE** LA PROPRIETA' PER CUI L'AVEVI SCELTO (orch, 2026-09-26, misurato sul mio waiter CI)
Questo file pretende gia' che il controllo a risposta nota stia **dentro** lo strumento e che il pos ctrl si scelga fra cose che **non possono** essere assenti. Manca la clausola temporale: **un soggetto valido all'arm puo' smettere di esserlo mentre il monitor gira**, e allora il controllo spara su uno strumento sano.
🔬 **Misurato.** Il waiter CI di #2299 portava un controllo d'orologio: *"il predicato deve contare ≥1 check-run STANTIO sulla head PRE-rebase"* — soggetto scelto bene, e alle `09:54Z` aveva risposto **`clock_ctrl=9`**. Ri-armato quaranta minuti dopo: **`CI-TOOL-DEAD clock_ctrl=0 (predicate blind)`**. Il predicato era intatto; **il SOGGETTO era sparito**: `gh api repos/O/R/commits/d07c9b4fe…` ⇒ **HTTP 422 `No commit found for SHA`**, perche' il force-push del rebase l'aveva reso irraggiungibile e GitHub l'ha lasciato cadere.
🥇 **Il verso giusto, e va detto prima della cura: il controllo HA FATTO IL SUO LAVORO.** Si e' rifiutato di armarsi invece di pubblicare verdetti con un predicato che non aveva esercitato. **Un `TOOL-DEAD` non e' un guasto da aggirare, e' il solo esito che distingue *"non ho misurato"* da *"ho misurato zero"*.** La tentazione — abbassare la soglia a `>=0`, o togliere il controllo per «sbloccare» il waiter — e' esattamente la mossa che trasforma un cancello in un timbro.
🔑 **CURA: si sceglie un soggetto la cui proprieta' e' garantita dalla SUA STORIA, non dal suo stato corrente.** La head di una PR **MERGIATA** e' raggiungibile per costruzione e i suoi check-run non si muovono piu'; la head di un ramo **VIVO** puo' essere riscritta da qualunque force-push, cioe' proprio dall'evento che il waiter sta aspettando. Misurato sul rimpiazzo: `d772e8fc9` (head mergiata di #2297) ⇒ **9 check-run, 9 stantii**, e `clock_ctrl=9` all'arm.
🥇 **Regola generale: quando scegli un soggetto di controllo, chiediti CHI PUO' TOGLIERGLI LA PROPRIETA' e se quel qualcuno e' proprio l'evento che stai misurando.** Se lo e', il controllo e' accoppiato al fenomeno e morira' nel momento peggiore — non a caso, ma **sistematicamente al successo**.
⚠️ **E nella diagnosi ho ripreso in pieno una trappola gia' a verbale qui, che vale ri-nominare perche' si presenta travestita da riga di debug:** `gh api <sha inesistente> -q .sha | head -2` ha stampato **il corpo d'errore JSON su STDOUT** e poi **`rc=0`** — che era l'rc di **`head`**, non della chiamata. Nello stesso giro `… -q '.check_runs[].started_at' | grep -c .` ha risposto **`1`**: il corpo d'errore contato come **una riga di dato**. ⇒ **in uno strumento, `gh api` si avvolge in una funzione che ritorna l'rc DELLA CHIAMATA e stampa solo su rc=0** — mai contare le righe del suo output senza aver letto l'rc:
```sh
api() { local o; o=$(gh api "$1" --paginate -q "$2" 2>/dev/null); local r=$?; [ $r -eq 0 ] && printf '%s\n' "$o"; return $r; }
```
🪞 *Ennesima faccia della famiglia, costume nuovo: non lo strumento rotto, non l'artefatto sbagliato, non il privilegio, non l'universo — ma **il metro di riferimento che e' stato cancellato dalla Storia mentre lo usavi**.*

## ⏳🔴 DUE ATTESE DIVERSE CHE CONDIVIDONO UN CONTATORE E UN MESSAGGIO: LA SECONDA ACCUSA IL SOGGETTO DELLA PRIMA (orch, 2026-09-26, difetto in un waiter MIO, beccato rileggendolo prima di ri-armarlo)
Il waiter CI di #2300 aspettava **due cose in sequenza**: prima che una worker **pushasse un fix** (head off da una sha nota-rossa), poi che **i check si settlassero**. Un contatore solo, `N`, e il messaggio di soglia scritto per la PRIMA attesa: *«head STILL 48c8f0e0c after ~30min — w2 has not pushed the shottino fix»*. In fondo al loop, `KNOWN_RED="$HEAD"` ⇒ **dopo che la head si era mossa, il giro dopo rientrava nel ramo d'attesa-push**, `N` ripartiva a salire e a ~30 min avrebbe emesso quella riga **su una worker che aveva pushato venti minuti prima**; a ~60 min avrebbe **abbandonato una CI viva** (`integration` sana dura 19-25 min ⇒ la soglia cadeva dentro la finestra normale).
🔑 **La direzione e' quella che costa:** il referto non tace, **ACCUSA** — e accusa il soggetto sbagliato. Letto al volo manda a inseguire una worker che non ha nessuna colpa, e la cura "giusta" che suggerisce (ri-mandare l'ordine) e' la **doppia sottomissione** che questo file registra come danno reale.
🥇 **REGOLA: un waiter che attraversa N FASI vuole N CONTATORI E N MESSAGGI.** Un contatore riusato non e' un'ottimizzazione: e' un'**asserzione che le due attese siano la stessa domanda**, e appena non lo sono il testo della soglia diventa una diagnosi falsa. Forma che regge: **il soggetto della fase 1 IMMUTABILE** (`ORIG_RED`, mai riassegnato — un assert `grep -c '^ORIG_RED='` = 1 e `grep -c 'KNOWN_RED='` = 0 lo pinna), una variabile SEPARATA (`SEEN`) per de-duplicare l'annuncio di transizione, e per la fase 2 **contatore proprio azzerato alla transizione** piu' una soglia derivata dalla **durata attesa di QUELLA fase**, non di quella prima.
🪞 **E il difetto l'ha trovato la RILETTURA, non un evento:** lo script era in volo da prima del mio `/clear` e l'avrei ri-armato identico. ⇒ **prima di ri-armare un waiter ereditato da una sessione precedente, RILEGGILO** — e in particolare chiediti se qualche variabile di stato viene riassegnata dentro il loop, perche' e' li' che un predicato cambia significato senza che il testo cambi.
🥇 *Parente dello «zero falso e plausibile» dal lato opposto: non un check che tace, ma un check che PARLA con la voce della fase sbagliata.*

## 🔬🔴 TRE CONTROLLI POSITIVI CHE NON POTEVANO RISPONDERE SI, IN UNA SESSIONE — E TUTTI E TRE DANNO LA COPPIA `pos=0 neg=0`, CHE QUESTO FILE CHIAMA GIA' «STRUMENTO MORTO» (orch, 2026-09-2x)
La regola *«il positivo si sceglie DIMOSTRANDO che risponde SI»* e' scritta in questo file da settimane. **L'ho violata tre volte nella stessa sessione**, ogni volta con un meccanismo diverso — il che dice che non e' distrazione, e' che **la scelta del positivo si fa a occhio mentre l'attenzione e' sul dato.**
1. **`pgrep -fl "pgrep"`** — per provare che la sonda di corsia fosse viva. **`pgrep` si esclude da solo** ⇒ muto **per costruzione**, e stavo per leggerlo come *"corsia occupata da nulla"*.
2. **Una fixture closing-keyword la cui terza riga era `owner/repo#N`** … **senza nessun verbo davanti.** `2 su 3` ⇒ ho accusato lo scanner, e il difetto era **nella fixture**.
3. **`grep -c 'PERDERE. LA PROPRIETA'` su `SKILL.md`** — il `.` non puo' matchare i **due asterischi** del grassetto ⇒ **0 su una sezione presente**.
🔑 **In tutti e tre il neg ctrl dava 0 pure lui**, cioe' la firma che questo file attribuisce allo strumento morto — quindi la coppia `pos=0 neg=0` **non distingue** *strumento rotto* da *positivo scelto male*, e sono diagnosi con cure opposte (riparare il comando vs cambiare il campione). **Il discriminante costa una chiamata: gira lo strumento SOSPETTO su un soggetto di cui hai gia' PROVATO la proprieta'.**
🥇 **E la clausola che becca il caso 2, che i primi due non avrebbero beccato: quando il positivo e' un INSIEME di grafie, il verdetto e' `hit == |insieme|`, MAI `hit > 0`.** Un matcher validato su un solo esemplare **tace esattamente sulle grafie che non conosce**, e il suo verde e' indistinguibile da quello vero.
🪞 🔴 **QUARTA ISTANZA, E L'HA PRODOTTA LA VERIFICA DI QUESTA STESSA LEZIONE, un minuto dopo averla scritta:** il pos ctrl della sezione dava **2 su 3** perche' la mia chiave era `pgrep. si esclude` — **un `.` dove il file ha un backtick**. Identica al caso 3, sul testo che la descrive. 🥇 **La cura non e' "stare piu' attenti": e' che una chiave di pos ctrl si COPIA DAL FILE, mai si digita a memoria** — e su un file pieno di markdown il carattere che ti frega e' sempre un delimitatore (backtick, asterisco), perche' quando rileggi la frase **il tuo occhio lo salta.** Rimisurato con chiavi copiate: **4 su 4**, neg ctrl 0 su due inventate.
🪞 *Il caso 3 e' il piu' istruttivo perche' il bersaglio era una riga che avevo scritto IO pochi minuti prima: sapevo che c'era, e il grep diceva no. **Quando cerchi una cosa di cui sei certo e lo strumento nega, il sospetto va sullo strumento — non e' fiducia in se' stessi, e' che li' hai una proprieta' PROVATA** ed e' il positivo migliore che ti capitera'.*

## 🚪🔴 UNA COSA GATATA **TRANSITIVAMENTE** NON PORTA NESSUNO DEI TOKEN CHE GREPPERESTI — E STAVOLTA L'ASSOLUZIONE ARRIVA DA UN REFERTO LE CUI MISURE TORNANO TUTTE (orch, 2026-09-26, #2300)
Ho gia' a verbale, dal lato mio, *«enumera i JOB, non grepare il token del gate»*: un job tenuto fermo da un `needs:` non porta ne' l'`if:` ne' il `push:` che cercavi. **Seconda istanza, dal lato opposto, e su un artefatto altrui.** Una worker ha censito gli specchi del wire e ha concluso che `docs/CLIENT_PROTOCOL.md` *«has a gate of its own that runs NOWHERE — not in check.sh, not in mix.exs, not in any CI workflow»*, e che la PR lo soddisfa *«by luck of habit, not because a gate asked»*. **Le sue tre grep sono VERE** (`grep -c 'client-protocol-gate'` ⇒ 0 su `scripts/check.sh`, 0 su `mix.exs`, 0 su `.github/workflows/`). **La conclusione e' FALSA:** quel documento e' gatato **in `check.sh` E in CI**, e la catena ha quattro anelli — `ci.yml:152` gira `bats test/bin/ test/infra/ test/scripts/`; `check.sh:40` chiama `bats.sh`; `bats.sh:57` gli passa `test/scripts/`; e `client_protocol_gate_test.bats:32-33` definisce `DOC="$REPO/docs/CLIENT_PROTOCOL.md"` — **il documento VERO** — passandolo in **2 casi su 7**, uno dei quali si chiama letteralmente `@test "the committed document satisfies the gate"`. Il gate default-a l'albero da se' (`DOC="${1:-docs/CLIENT_PROTOCOL.md}"`).
🔑 **Il meccanismo, e non e' sciatteria di chi ha misurato: il gate e' raggiunto UN'INDIREZIONE SOTTO, attraverso una SUITE che lo invoca, quindi il suo nome non compare in nessuno dei posti dove lo cercheresti — per costruzione.** ⇒ **la domanda non e' «chi lo NOMINA» ma «chi lo ESEGUE, anche di rimbalzo»**, e si risponde **risalendo i chiamanti**, mai cercando la stringa.
🪞🔴 **E la meta' che riguarda me: stavo per "correggere" una riga MIA che era GIUSTA.** Il mio handoff diceva *«gira in LOCALE»*; il referto arrivava con tre misure tutte verificabili e tutte vere, e la tentazione di incassarlo era massima **proprio perche' le misure tornavano**. ⇒ 🥇 **quando un referto ti smentisce una riga, verifica la CATENA che la sua conclusione nega, non le sue misure** — quelle possono essere impeccabili e rispondere a un'altra domanda. *Un'inferenza sbagliata poggiata su misure giuste e' la forma piu' persuasiva che esista: e' gia' passata il controllo che ti verrebbe in mente di fare.*
💰 **Il costo evitato non era cosmetico:** incassata, avrei filato in issue 2262 una modifica di processo che **aggiunge a `check.sh` e a CI un gate presente in entrambi**, e avrei portato il conteggio degli assenti da tre a quattro sbagliando il quarto.
🪞🔴 **E IL «TRE» DI QUELLA FRASE ERA GIA' SBAGLIATO, PER LO STESSO DIFETTO, MENTRE LA SCRIVEVO: SONO DUE (misurato 2026-09-26 dal censimento di w2, verificato da me in proprio).** `scripts/design-notes-gate.sh` **gira da `check.sh`, sull'albero VERO**: `check.sh:40` → `bats.sh` → `bats.sh:57` (`set -- test/bin/ test/infra/ test/scripts/`) → `design_notes_gate_test.bats:492`, `@test "the real docs/DESIGN_NOTES.md passes the gate on this branch"`, che a `:495` fa `cd "$REAL_REPO"` e gira il gate NUDO — osservato come `ok 715` in un run reale. Corretti body, titolo e un commento su issue 2262.
🥇 **E i due assenti veri lo sono per una ragione PIU' AFFILATA di «non e' chiamato»: `check.sh` LI RAGGIUNGE entrambi via bats, ma ogni tocco sull'albero vero e' `--list`.** `shellcheck.sh`: 4 usi di `$GATE` = **2 `cp` in sandbox + 2 `--list`** ⇒ **il LINT non gira**. `posix-parse.sh`: copie sandbox piu' un solo `--list | grep -qxF` (`packaging_prerelease_marker_test.bats:236`) ⇒ **il PARSE non gira**. **`--list` e' il gate che risponde a una domanda SU DI SE', non che giudica l'albero** ⇒ un censimento che contasse *«lo script e' stato ESEGUITO»* li darebbe tutti e tre coperti, **e sbaglierebbe nella direzione che sembra sicura.**
🥇🥇 **LA LEZIONE CHE NE ESCE E' PIU' GRANDE DI QUESTA PAGINA, E NESSUN CONTROLLO LA BECCA: UNA LEZIONE PUO' ESSERE GIUSTA E PORTARE DENTRO DI SE' UN NUMERO PRODOTTO DAL DIFETTO CHE DESCRIVE.** Avevo appena scritto *«la domanda non e' chi lo NOMINA ma chi lo ESEGUE»* e nella riga dopo ho citato come base un conteggio che era **esattamente un conteggio di NOMI**. ⇒ **quando scrivi una lezione su una classe di difetto, la PRIMA cosa da rimisurare sono i NUMERI CHE LA LEZIONE STESSA CITA**: e' il posto dove il difetto e' piu' probabile e meno sospettato, perche' quei numeri arrivano dal contesto in cui l'hai scoperto e li senti come lo sfondo del ragionamento, non come un'affermazione. ⚠️ **E vale DI PIU' per un numero prodotto da un ALTRO** — il tre non era mio: *«non e' mio» spiega perche' non l'ho rimisurato, non perche' l'ho citato.*
🥇🥇 **LO SCHEMA, e l'ha nominato la worker meglio di come l'avevo corretto io — quattro istanze, due dentro il suo stesso censimento: «affermare che niente impone X, su una misura di qualcosa di ADIACENTE all'imposizione, mentre la smentita sta in materiale che ho GIA' in mano».** (a) *"nessun gate lega shottino al server"* ← misurati gli **EDIT** (40 commit) e concluso l'**IMPOSIZIONE**, smentita nella seconda meta' del subject del commit citato come prova; (b) *"il gate gira NOWHERE"* ← misurato chi **NOMINA**, smentita alla riga 13008 del suo log di `check.sh`; (c) il **tre** di 2262, che **non era suo** — *"lo schema non e' un tic personale"*; (d) il suo **primo pos ctrl**, che asseriva la **PRESENZA** dove la tesi richiedeva l'**ASSE**, ed **e' passato da cieco**. 🔑 **IL TELL, riusabile: misura e affermazione parlano di GRANDEZZE DIVERSE** — edit vs imposizione, menzioni vs esecuzione, presenza vs asse, enumerazione vs giudizio. **Davanti a un negativo che poggia su un conteggio: nomina la grandezza che il conteggio MISURA e quella che l'affermazione RICHIEDE, e verifica che siano la stessa.** 🥇 *E non ha rivendicato che quattro sia il conteggio: "non ho spazzato il mio output precedente cercando lo schema". Chiedi quella clausola nei brief.*
🥇 **E la sua meta' migliore, da chiedere nei brief: nello STESSO referto lo stesso errore era gia' scritto e RITRATTATO su un altro gate** (*«this is the gate I wrongly said did not exist»*, su shottino) **senza essere generalizzato.** ⇒ **quando ti becchi un'inferenza sbagliata, la domanda successiva e' sempre «quante altre istanze hanno QUESTA FORMA?»** — li' ce n'era una a due pagine di distanza.
✅ **Nota a credito del metodo: il suo pos ctrl della sonda per-numero ha beccato una SONDA MORTA** — la forma iniziale usava `\b`, che l'ERE di `git grep` **non onora**, e stampava un vuoto **identico a «nessuna copia da nessuna parte»** (il trap POSIX-ERE gia' a verbale in questo file). Il controllo era DENTRO lo strumento e ha girato PRIMA dei numeri: e' per questo che il suo zero sul quarto specchio vale qualcosa.
