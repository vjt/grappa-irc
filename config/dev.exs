...
# over the base :admission block). The #171 cap reuses `max_per_client`,
# and the serial Playwright suite drives every browser login through ONE
# shared source IP (the e2e nginx) and every direct API login through
# another (the runner) — many DISTINCT seeded subjects per IP. At the
# production default of 1, user `/connect` and visitor logins from the
# 2nd subject on a shared IP would 503 `too_many_sessions`, cascading
# unrelated specs. 10 matches the seeded bahamut-test user-session
# ceiling, so the per-IP cap is never tighter than the network total in
# e2e. Networks that need a tight cap set `max_per_client` explicitly
# (u-3, u-4, the #171 spec); azzurra seeds 100 for anon-visitor volume.
config :grappa, :admission, default_max_per_client_per_network: 10
