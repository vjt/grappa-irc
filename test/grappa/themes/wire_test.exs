defmodule Grappa.Themes.WireTest do
  ...
  alias Grappa.Repo
  alias Grappa.Themes
  alias Grappa.Themes.{Theme, TokenModel, Wire}
  ...
  describe "to_wire/2" do
    test "renders the full wire shape with the owner's name as author" do
      ...
      {:ok, theme} = Themes.create_theme({:user, user}, %{name: "Night", payload: valid_payload()})
      {:ok, theme} = Themes.get_theme(theme.id)
    ...
    test "mine is true only when the viewer is the owner" do
      owner = user_fixture()
      other = user_fixture()
      {:ok, theme} = Themes.create_theme({:user, owner}, %{name: "N", payload: valid_payload()})
      {:ok, theme} = Themes.get_theme(theme.id)
