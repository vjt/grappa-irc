...
- **PWA home-screen icon badge (2026-06-21)** — the installed app's launcher icon shows a count (0–99) of unread messages the operator chose to be *notified* about: the same predicate as Web Push (DMs / channels / mentions per notification prefs), server-derived from read cursors, cleared by reading. Three doors feed it — the `/me` boot seed, the `read_cursor_set` cross-device broadcast, and the push payload (so the service worker stamps the icon while the app is closed) — plus a `document.title` `(n) …` mirror for desktop. Same notify set as the OS push by construction (one reused predicate, `Grappa.Push.Triggers.should_notify?/4`); no new persisted state.
