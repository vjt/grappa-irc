defmodule Grappa.QueryWindows do
  ...
  Both `open/3` and `close/3` are safe to call multiple times with the
  same arguments. `open/3` returns the existing row on a duplicate call
  without updating it ("first opened" semantics — `opened_at` is a
  stable anchor). `close/3` returns `:ok` whether or not a row existed.

  ## Visitor sessions

  Visitor sessions should NOT call `open/3` (spec line 46: "Skipped for
  ...
  """
  @spec open(user_id :: Ecto.UUID.t(), network_id :: integer(), target_nick :: String.t()) ::
          {:ok, Window.t()} | {:error, Ecto.Changeset.t()}
  def open(user_id, network_id, target_nick)
      when is_binary(user_id) and is_integer(network_id) and is_binary(target_nick) do
  ...
  Case-insensitive: `close(u, n, "FooBar")` deletes a "foobar" window.
  Returns `:ok` whether or not a row was deleted (idempotent).
  """
  @spec close(user_id :: Ecto.UUID.t(), network_id :: integer(), target_nick :: String.t()) ::
          :ok
  def close(user_id, network_id, target_nick)
      when is_binary(user_id) and is_integer(network_id) and is_binary(target_nick) do
