defmodule Grappa.Deploy.Preflight do
  @moduledoc """
  Single source of truth for `scripts/deploy.sh` hot-vs-cold classification.
  ...
  * **H20** — the prior bash regex missed several path classes whose
  ...
  @doc """
  Classify a list of changed paths. Does NOT exercise per-file content
  diffs for long-lived-module state-shape checks — use `classify/3`
  with the from/to revs for that.

  Returns `{:hot, []}` when no path triggers a cold class; otherwise
  `{:cold, [reasons]}` enumerating every triggered class.
  """
  @spec classify_paths([String.t()]) :: verdict()
  def classify_paths(paths) when is_list(paths) do
    reasons =
      ...
      |> add_reason(:image_substrate, Enum.filter(paths, &image_substrate?/1))
  ...
  @doc """
  CLI entry point invoked by `scripts/deploy.sh` via `mix run -e
  'Grappa.Deploy.Preflight.cli(System.argv())'`.

  Expects exactly two args: `from_sha` and `to_sha`. Shells out to
  `git diff --name-only` for the changed-paths list and to
  `git show <rev>:<path>` for state-shape checks on long-lived
  modules.
  ...
  def cli([from, to]) when is_binary(from) and is_binary(to) do
    case classify(from, to, &git_diff_paths/2, &git_show/2) do
      {:hot, []} ->
  ...
  def cli(_) do
    IO.puts(:stderr, "usage: mix run -e 'Grappa.Deploy.Preflight.cli([from_sha, to_sha])'")
    System.halt(2)
  end

  @doc """
  Classify a git diff range. Uses injected callbacks for git shell-out
  (so the module is testable without git).
  ...
  * `show_fn.(rev, path)` → `String.t() | nil` — full contents of `path`
  ...
  def classify(from, to, diff_paths_fn, show_fn) do
    paths = diff_paths_fn.(from, to)

    case classify_paths(paths) do
      {:cold, _} = cold ->
        cold
  ...
  defp image_substrate?("compose.override.yaml"), do: true
  defp image_substrate?("compose.override.yaml.example"), do: true
